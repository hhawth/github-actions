...
def year_weightings(records, home=True, previous_year_weight=0.4, current_year_weight=0.6):
    old = None
    current = None
    for row in records:
    ...
    scored_avg_weighted = old[0] * previous_year_weight + current[0] * current_year_weight
    scored_max_weighted = old[1] * previous_year_weight + current[1] * current_year_weight
    conceded_avg_weighted = old[2] * previous_year_weight + current[2] * current_year_weight
    conceded_max_weighted = old[3] * previous_year_weight + current[3] * current_year_weight
    return scored_avg_weighted, scored_max_weighted, conceded_avg_weighted, conceded_max_weighted
...
def get_fixtures():
    ...
    for fixture in fixtures:
        ...
        odds[0].text.split(" ")[1].split("/"), odds[1].text.split("/"), odds[2].text.split(" ")[1].split("/")
        home_wins_odds = odds[0].text.split(" ")[1].split("/")
        home_wins_odds_as_percent = int(home_wins_odds[1]) / (int(home_wins_odds[0]) + int(home_wins_odds[1]))

        draw_odds = odds[1].text.split("/")
        draw_odds_as_percent = int(draw_odds[1]) / (int(draw_odds[0]) + int(draw_odds[1]))
        ...
        home_scored_avg, home_scored_max, conceded_home, conceded_home_max = year_weightings(records)

        cur.execute(f"SELECT * from goals WHERE team = '{away_team}'")
        records = cur.fetchall()
        away_scored_avg, away_scored_max, conceded_away, conceded_away_max = year_weightings(records, home=False)

        home_score_guess = estimate_value(home_scored_avg, home_scored_max, conceded_away, conceded_away_max, home_wins_odds_as_percent, away_wins_odds_as_percent)
        away_score_guess = estimate_value(away_scored_avg, away_scored_max, conceded_home, conceded_home_max, home_wins_odds_as_percent, away_wins_odds_as_percent, home = False)
